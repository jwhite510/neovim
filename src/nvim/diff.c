...
void count_filler_lines_and_topline(int *curlinenum_to, int *linesfiller, const diff_T *thistopdiff,
                                    const int toidx, int virtual_lines_passed)
{
...
void run_linematch_algorithm(diff_T *dp)
{
  ...
  for (int i = 0; i < DB_COUNT; i++) {
    ...
    if (curtab->tp_diffbuf[i] != NULL) {
      ...
      // keep track of the index of the diff buffer we are using here with outputmap
      // ( 0 >= i < DB_COUNT )
      // we will use this to write the output of the algorithm to diff_T structs at the
      // correct indexes
